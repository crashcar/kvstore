...
// 使用 ENABLE_KEY_CHAR 宏定义方便直接修改rbtree的存储类型
#if ENABLE_KEY_CHAR
...
typedef struct _rbtree_node {
	...
	KEY_TYPE key; // KEY_TYPE 宏定义方便直接修改存储的类型
	void *value;
} rbtree_node;
...
// 5个对外接口：set、get、modify、delete、count
// 接口函数set的时候才使用kvstore_malloc创建节点，而在上面的rbtree操作函数是不需要创建节点的，直接接受节点地址为参数即可
